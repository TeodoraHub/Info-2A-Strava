# Point Hebdomadaire - Projet Application de sport

Date : Jeudi 7 novembre 2025
Semaine n° 10

## Tâches réalisées cette semaine
- [x] rc/utils/gpx_parser.py** ✅ 
    Parser complet pour fichiers GPX:
- `parse_gpx_file()` - Parser un fichier GPX complet
- `_extract_track_points()` - Extraire les points de trace
- `_parse_track_point()` - Parser un point individuel
- `_calculate_total_distance()` - Calculer la distance totale
- `_haversine_distance()` - Calculer distance entre 2 points GPS
- `_calculate_duration()` - Calculer la durée
- `get_elevation_gain()` - Calculer le dénivelé

- [x] src/utils/securite.py
    Fonctions de sécurité renforcées:
- `hash_password()` - Hashage SHA256 (ancienne version, conservée)
- `hash_password_bcrypt()` - Hashage bcrypt (recommandé) 
- `verify_password_bcrypt()` - Vérifier mot de passe bcrypt
- `generate_secure_token()` - Générer token sécurisé 
- `valider_force_mot_de_passe()` - Valider force du mot de passe 
- `sanitize_input()` - Nettoyer les entrées utilisateur 

- [x] src/API.py** ✅ 
API REST  avec la majorité endpoints:
- Endpoints Utilisateurs
- Endpoints Activités
- Endpoints Likes
- Endpoints Commentaires
- Endpoints Utilitaires


### CORNET--CARLOS Abel
- [x] Corrections des différentes parties du code pour faire fonctionner les endpoints
- [x] 

### GAUTIER Victor
- [x] Création de l'interface en ligne de commandes
- [x] Implémentation de l'import de fichier gpx
- [x] Création de l'interface utilisateur
- [x] Corrections de code

### MOLDOVAN Teodora

- [x] première version de code pour les Vues activités
- [x] tester les endpoints dans API

### WEBER Grégoire
- [x] creation template rapport
- [x] passage de langage sql à langage sqlalchemy
      
### YABOYA Anicet Marius
- [x] Premiers tests avec streamlit
- [x]  réflexion sur le modèle phyique
- [x] Prise en main avec SQLAlchemy
- [x] Tests des endpoints dans l'API
- [x] correction de code     
